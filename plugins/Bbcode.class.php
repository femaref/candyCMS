...
=======
			...
			$sStr = preg_replace('#\[audio\](.*)\[\/audio\]#Uis', '<div class="js-media" title="' . $sUrl . '"><a href="' . $sUrl . '" class="js-media">' . $aMatch[1] . '</a></div>', $sStr);
		}

		# [video]file[/video]
		if (preg_match('#\[video\](.*)\[\/video\]#Uis', $sStr, $aMatch)) {
			$sUrl = 'http://url2video.com/?url=' . $aMatch[1] . '&w=' . MEDIA_DEFAULT_X . '&h=' . MEDIA_DEFAULT_Y;
			$sStr = preg_replace('#\[video\](.*)\[\/video\]#Uis', '<div class="js-media" title="' . $sUrl . '"><a href="' . $aMatch[1] . '">' . $aMatch[1] . '</a></div>', $sStr);
		}

		# [video thumbnail]file[/video]
		if (preg_match('#\[video (.*)\](.*)\[\/video]#Uis', $sStr, $aMatch)) {
			$sUrl = 'http://url2video.com/?url=' . $aMatch[2] . '&w=' . MEDIA_DEFAULT_X . '&h=' . MEDIA_DEFAULT_Y . '&p=' . $aMatch[1];
			$sStr = preg_replace('#\[video (.*)\](.*)\[\/video]#Uis', '<div class="js-media" title="' . $sUrl . '"><a href="' . $aMatch[2] . '">' . $aMatch[2] . '</a></div>', $sStr);
		}

		# [video width height thumbnail]file[/video]
		if (preg_match('#\[video ([0-9]+) ([0-9]+) (.*)\](.*)\[\/video\]#Uis', $sStr, $aMatch)) {
			$sUrl = 'http://url2video.com/?url=' . $aMatch[4] . '&w=' . $aMatch[1] . '&h=' . $aMatch[2] . '&p=' . $aMatch[3];
			$sStr = preg_replace('#\[video ([0-9]+) ([0-9]+) (.*)\](.*)\[\/video\]#Uis', '<div class="js-media" title="' . $sUrl . '"><a href="' . $aMatch[4] . '" class="js-media">' . $aMatch[4] . '</a></div>', $sStr);
		}
...
>>>>>>> 18e7f813f23da28d17ab640f5bc8cb7c6b1d776a
}