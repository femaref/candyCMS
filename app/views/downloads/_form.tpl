<form action='/download/{$smarty.get.action}' method='post' enctype='multipart/form-data'>
  ...
  <p {if isset($error_category)}class="error" title="{$error_category}"{/if}>
    <label for='input-category'>{$lang.global.category} *</label>
  ...
  <p class="center">
    ...
    {if $smarty.get.action == 'update'}
      <input type='button' value='{$lang.global.destroy.destroy}' onclick="confirmDelete('/download/{$_request_id_}/destroy')" />
