...
class Dispatcher {
   ...
   * @see vendor/candyCMS/controllers/Main.controller.php -> __autoload()
   *
   */
  public function getController() {
    ...
    try {
      ...
      elseif (file_exists(PATH_STANDARD . '/vendor/candyCMS/controllers/' . $sController . '.controller.php')) {
        require_once PATH_STANDARD . '/vendor/candyCMS/controllers/' . $sController . '.controller.php';

        $sClassName = '\CandyCMS\Core\Controller\\' . $sController;
        $this->oController = new $sClassName($this->_aRequest, $this->_aSession, $this->_aFile, $this->_aCookie);
      }
        ...
        if(substr(strtolower($sController), 0, 6) !== 'upload')
          throw new AdvancedException('Controller not found:' . PATH_STANDARD .
                  '/vendor/candyCMS/controllers/' . $sController . '.controller.php');
      }
    }
